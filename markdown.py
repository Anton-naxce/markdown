...
#1. map_schemas(schema1_file, schema2_file, mapping_file) - Эта функция принимает две схемы XSD в качестве входных данных,
#сравнивает их элементы и создает файл отображения в формате XML (.mfd), чтобы отслеживать отображение между исходными и целевыми полями.

#2. mapping_to_md(mapping_file, md_file) - Эта функция принимает файл отображения (.mfd) в качестве входных данных и создает таблицу
#в формате markdown для визуализации отображения между исходными и целевыми полями с любым соответствующим преобразованием.

#3. validate_xml(schema_file, xml_file) - Эта функция принимает схему XSD и XML-файл в качестве входных данных,
#проверяет XML-файл на соответствие заданной схеме XSD, и возвращает, является ли XML-файл допустимым или нет.
#1. mapping_to_md() - преобразует файл маппинга (в формате .mfd) в таблицу Маркдаун (в формате .md).
#2. validate_xml() - проверяет, соответствует ли документ в формате XML заданной XSD-схеме.
#3. map_schemas() - выполняет маппинг элементов между двумя XSD-схемами и записывает их в файл (в формате .mfd).
